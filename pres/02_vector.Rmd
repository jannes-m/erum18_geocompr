...
For example, let us add the region name from `nz` to `nz_height` (so far consisting of columnes `t50_fid`, `elevation` and `geometry`).
--
...
We display spatial data in 2D, i.e., we neglect the curvature of the Earth which leads to distortions.
We distinguish between:

- geographical Coordinate Reference Systems (CRS) use one specific ellipsoid (WGS84)
- projected CRS (usually use a specific ellipsoid which is especially suitable for a certain part of the Earth)
...
Reproject with the help of `st_transform()`:

```{r, eval=FALSE}
st_transform(us_states, crs = 4326)
```

Further reading: [https://geocompr.robinlovelace.net/transform.html#reproj-geo-data](https://geocompr.robinlovelace.net/transform.html#reproj-geo-data)
...
Further reading: [https://geocompr.robinlovelace.net/transform.html#geo-vec](https://geocompr.robinlovelace.net/transform.html#geo-vec)
---
...
- Find out about the CRS of `nz`, reproject it into a geographic CRS (EPSG: 4326) and plot the original and unproject `nz` object next to each other.
